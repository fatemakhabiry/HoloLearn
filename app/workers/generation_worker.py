...
async def run_generation(ctx: dict, lecture_id: int, audio_path: str = None) -> None:
    """
    ARQ background job — runs hololearn_pipeline.py for a single lecture.
    ...
    NOTE: audio_path is now accepted (run_tts enqueues this job with it as
    of the TTS/transcript feature) but NOT YET USED below — this function
    still runs the ORIGINAL local-subprocess flow against PIPELINE_SCRIPT,
    which does its own TTS internally via LONGCAT_ENV_PYTHON and ignores
    audio_path entirely. This is intentional for now: the GPU machine is
    unreachable, so the HTTP-based rewrite (sending audio_path to the GPU
    machine instead of running TTS+avatar generation here) is deliberately
    not deployed yet, pending a confirmed contract with the GPU-side app.

    This signature change exists ONLY to stop a TypeError crash —
    run_tts was calling enqueue_job("run_generation", lecture_id, audio_path)
    against the old 2-arg signature. Default value (= None) keeps this
    callable without audio_path too, in case anything else still enqueues
    it the old way.

    Practical effect right now: when GPU_PIPELINE_ENABLED chains into this
    job, it will hit the "Pipeline not configured in .env" guard below
    (since LONGCAT_ENV_PYTHON/PIPELINE_SCRIPT point at the unreachable GPU
    setup) and fail cleanly via _fail() — which is fine for transcript-only
    testing, since GPU_PIPELINE_ENABLED should be False during that anyway
    and this job won't even be enqueued in that case.
    """
    with Session(engine) as session:
