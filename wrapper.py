...
class GraphRAGWrapper:
    ...
    def query(
        ...
        # Determine whether this was a voice query, and resolve the
        # actual transcribed text (falls back to the original text query)
        is_audio = graded_result.trace.input_modality == "audio"
        resolved_query_text = (
            graded_result.trace.transcription_text
            if is_audio and graded_result.trace.transcription_text
            else query_text
        )
        ...
        try:
            pipeline.record_turn(
                user_query    = resolved_query_text,
                ai_response   = answer,
                graded_result = graded_result,
                audio         = is_audio,
            )
